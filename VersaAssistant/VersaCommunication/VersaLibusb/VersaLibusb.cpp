...
void VersaLibusb::run()
{
	...
	while (!isInterruptionRequested())
	{
		if (libusbOpenState == true)
		{
            ...
            {
                qDebug()<<"libusbOpenState"<<result;
            }

            if((result == LIBUSB_ERROR_IO)||(result==LIBUSB_ERROR_NO_DEVICE) )
            {
                closeLibusbHid();
                qDebug()<<"readyRead"<<result;
                emit errorClose();
            }
			else if ((result == LIBUSB_SUCCESS)&&(bytesRead > 0))
			{
...
bool VersaLibusb::writeBuffer(const QByteArray& data)
{
	...
	while (offset < data.size())
	{
		...
		// 获取 QByteArray 的数据指针
		const char* databuffer = data.constData();
		memset(buffer, 0, listUsbInfo[portIndex].MaxPacketSizeOut);
		// 复制数据到 buffer
		memcpy(buffer, databuffer + offset, length);
